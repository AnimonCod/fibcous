...
async function compileCSS () {
  ...
  const files = await glob('packages/{,@fibcous/}*/src/style.scss')
  ...
  for (const file of files) {
    ...
    if (outputDir.includes(path.normalize('packages/fibcous/'))) {
      outfile = path.join(outputDir, 'fibcous.css')
    }
    await mkdir(outputDir, { recursive: true })
    await writeFile(outfile, postcssResult.css)
    console.info(
      chalk.green('✓ Built fibcous CSS:'),

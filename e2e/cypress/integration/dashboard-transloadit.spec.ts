describe('Dashboard with Transloadit', () => {
  ...
  it('should upload cat image successfully', () => {
    ...
    cy.get('.fibcous-StatusBar-actionBtn--upload').click()
    cy.wait(['@assemblies', '@resumable']).then(() => {
      cy.get('.fibcous-StatusBar-statusPrimary').should('contain', 'Complete')
    })
  })
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ fibcous }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(fibcous.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(fibcous.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ fibcous }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      fibcous.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ fibcous }) => {
      ...
      const { files } = fibcous.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      fibcous.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ fibcous }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(fibcous.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ fibcous }) => {
      const { files } = fibcous.getState()
      fibcous.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        fibcousserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'fibcous-core:3.2.0,fibcous-transloadit:3.1.3,fibcous-tus:3.1.0,fibcous-dropbox:3.1.1,fibcous-box:2.1.1,fibcous-facebook:3.1.1,fibcous-google-drive:3.1.1,fibcous-instagram:3.1.1,fibcous-onedrive:3.1.1,fibcous-zoom:2.1.1,fibcous-url:3.3.1',
    ...
    cy.get('.fibcous-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.fibcous-StatusBar-actions > .fibcous-c-btn').click()
    cy.wait([

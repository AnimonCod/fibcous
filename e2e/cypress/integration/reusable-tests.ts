...
export function runRemoteUrlImageUploadTest() {
  ...
  cy.get('.fibcous-Url-input').type(
    'https://raw.githubusercontent.com/transloadit/fibcous/main/e2e/cypress/fixtures/images/cat.jpg',
  )
...
export function runRemoteUnsplashUploadTest() {
  ...
  cy.get('.fibcous-SearchProvider-input').type('book')
  cy.intercept({
    method: 'GET',
    url: 'http://localhost:3020/search/unsplash/list?q=book',
  }).as('unsplash-list')
  cy.get('.fibcous-SearchProvider-searchButton').click()
  cy.wait('@unsplash-list')
  // Test that the author link is visible
  cy.get('.fibcous-ProviderBrowserItem')
    .first()
    .within(() => {
      cy.root().click()
  ...
  cy.get('.fibcous-c-btn-primary').click()
  cy.intercept({
    method: 'POST',

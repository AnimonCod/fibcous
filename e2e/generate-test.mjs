...
const packageNames = await fs.readdir(new URL('../packages/@fibcous', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@fibcous/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@fibcous/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('fibcous', 2)[1]

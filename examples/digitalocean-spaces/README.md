...
This example uses fibcous to upload files to a [DigitalOcean Space](https://digitaloceanspaces.com/).
DigitalOcean Spaces has an identical API to S3, so we can use the
[AwsS3](https://fibcous.io/docs/aws-s3-multipart) plugin. We use @fibcous/companion with a
...
`@fibcous/companion` in a `.env` file. You may find useful to first copy the
`.env.example` file:
...
[the plugin docs](https://fibcous.io/docs/aws-s3-multipart/#setting-up-your-s3-bucket),
...
```bash
corepack yarn workspace @fibcous-example/digitalocean-spaces start
```
